...
const Home = () => {
  ...
  const charPositions = useRef<{ x: number; y: number; rotation: number }[]>(
    Array(welcomeText.length).fill(null).map(() => ({ x: 0, y: 0, rotation: 0 }))
  );
  const charVelocities = useRef<{ vx: number; vy: number; vr: number }[]>(
    Array(welcomeText.length).fill(null).map(() => ({ vx: 0, vy: 0, vr: 0 }))
  );
  const charHit = useRef<boolean[]>(
    Array(welcomeText.length).fill(false)
  );
